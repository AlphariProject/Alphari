...
namespace Alphari_v2.ATA
{
	...
	public class ATA_PIO : Cosmos.HAL.BlockDevice.Ata
	{
        #region Properties
	    ...
	    internal static Debugger mDebugger = new("0", "AtaPio");
        ...
        public ATA_PIO(ATGIO aIO, Ata.ControllerIdEnum aControllerId, Ata.BusPositionEnum aBusPosition)
		{
			Console.WriteLine("aIO!");
			IO = aIO;
			Console.WriteLine("mControlledID!");
			mControllerID = aControllerId;
			Console.WriteLine("BusPos@BPOS");
			mBusPosition = aBusPosition;

			Console.WriteLine("DevWrite@CONTROL");
			aIOPort.Wr8(IO.Control, 0x02);

			Console.WriteLine("SYS@DISCOVER");
            mDriveType = DiscoverDrive();
			Console.WriteLine("DINIT@DRIVE");
			if (mDriveType != SpecLevel.Null)
			{
				Console.WriteLine("FINALY:INIT");
				InitDrive();
			}
		}
		...
		public SpecLevel DiscoverDrive()
		{
			...
			if (aIOPort.Rd8(IO.Status) == 0xFF)
			{
				Console.WriteLine("Spec: NULL");
				return SpecLevel.Null;
			}

            Console.WriteLine("IDENT:SEND");
		...
		{
            aIOPort.Wr8(IO.DeviceSelect, (byte)((byte)(DvcSelVal.Default | DvcSelVal.LBA | (mBusPosition == BusPositionEnum.Slave ? DvcSelVal.Slave : 0)) | aLbaHigh4));
            Wait();
		}
		...
		{
            aIOPort.Wr8(IO.Command, (byte)aCmd);
			Status xStatus;
			do
			{
				Wait();
				xStatus = (Status)aIOPort.Rd8(IO.Status);
		...
		protected void InitDrive()
		{
			...
			{
				Console.WriteLine("SEND:IDENTIFY");
				SendCmd(Cmd.Identify);
			}
			else
			{
                Console.WriteLine("SEND:IDENTIFYPACK");
            ...
            Console.WriteLine("READ:IDENTSPACE");
            for (int i = 0; i < 256; i++)
			{
                xBuff[i] = aIOPort.Rd16(IO.Data);
            }
            Console.WriteLine("GET:SERIALNO");
            mSerialNo = GetString(xBuff, 10, 20);
            Console.WriteLine("GET:FWREV");
            mFirmwareRev = GetString(xBuff, 23, 8);
            Console.WriteLine("GET:MODELNO");
            ...
            Console.WriteLine("GET:BLOCKCOUNT");
            ...
            Console.WriteLine("GET:LBA48BIT");
            LBA48Bit = (xBuff[83] & 0x40) != 0;
            // LBA48Bit = mBlockCount > 0x0FFFFFFF;
            if (LBA48Bit)
            {
                Console.WriteLine("GET:BLOCKCOUNT from LBA48BIT");
                ulong high = xBuff[102];
                ulong mid = xBuff[101];
                ulong low = xBuff[100];

                mBlockCount = (low | (mid << 16) | (high << 32)) - 1;
            }
        }
		...
		protected void SelectSector(ulong aSectorNo, ulong aSectorCount)
		{
			...
			if (LBA48Bit)
			{
                aIOPort.Wr16(IO.SectorCount, (ushort)aSectorCount);
                aIOPort.Wr8(IO.LBA0, (byte)(aSectorNo >> 24));
                aIOPort.Wr8(IO.LBA0, (byte)aSectorNo);
                aIOPort.Wr8(IO.LBA1, (byte)(aSectorNo >> 32));
                aIOPort.Wr8(IO.LBA1, (byte)(aSectorNo >> 8));
                aIOPort.Wr8(IO.LBA2, (byte)(aSectorNo >> 40));
                aIOPort.Wr8(IO.LBA2, (byte)(aSectorNo >> 16));
			}
			else
			{
				// Number of sectors to read
                aIOPort.Wr8(IO.SectorCount, (byte)aSectorCount);
                aIOPort.Wr8(IO.LBA0, (byte)aSectorNo);
                aIOPort.Wr8(IO.LBA1, (byte)(aSectorNo >> 8));
                aIOPort.Wr8(IO.LBA2, (byte)(aSectorNo >> 16));
		...
		public override void ReadBlock(ulong aBlockNo, ulong aBlockCount, ref byte[] aData)
        {
            ...
            int totalWords = (int)(aBlockCount * 256);
            ushort[] tempBuffer = new ushort[totalWords];

            for (int i = 0; i < totalWords; i++)
            {
                tempBuffer[i] = aIOPort.Rd16(IO.Data);
            }

            Buffer.BlockCopy(tempBuffer, 0, aData, 0, aData.Length);
        }
		...
		public override void WriteBlock(ulong aBlockNo, ulong aBlockCount, ref byte[] aData)
        {
            ...
            int totalWords = (int)(aBlockCount * 256);
            ushort[] tempBuffer = new ushort[totalWords];
            Buffer.BlockCopy(aData, 0, tempBuffer, 0, aData.Length);

            for (int i = 0; i < totalWords; i++)
            {
                aIOPort.Wr16(IO.Data, tempBuffer[i]);
            }
